...
if [ ! -f "/srv/ssl.key" ]; then
  /root/.acme.sh/acme.sh --issue --alpn --tlsport 443 --days 1 -d ${DOMAIN} --keylength ec-256 --standalone -m ${EMAIL} --force --fullchain-file /srv/fullchain.cer --key-file /srv/ssl.key
