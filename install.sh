...
install_xray() {
  ...
  echo "正在启动docker容器..."
  bash -c "docker run --name xray -it --restart=always --pull=always -p 443:443 -e PORT=443 -e DOMAIN=${DOMAIN} -e EMAIL=${EMAIL}  kingfalse/onekey-docker-xray /srv/xray-server.sh" | tee install.log
